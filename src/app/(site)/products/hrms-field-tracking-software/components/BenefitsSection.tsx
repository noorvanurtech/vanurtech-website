...
const benefits = [
  { title: "Eliminate Attendance Fraud Completely", icon: <ShieldCheck className="w-6 h-6" /> },
  { title: "Save 10+ Hours on Payroll Every Month", icon: <Clock className="w-6 h-6" /> },
  { title: "Boost Field Team Productivity by 40%", icon: <TrendingUp className="w-6 h-6" /> },
  { title: "Reduce HR Operational Costs", icon: <DollarSign className="w-6 h-6" /> },
  { title: "Keep Your Entire Team Accountable", icon: <Users className="w-6 h-6" /> },
];
        ...
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-24 items-center">
          ...
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="order-1 text-center lg:text-left">
            ...
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-5 sm:mb-8 text-white leading-[1.1] tracking-tight">
              How Our HRMS <br className="hidden md:block" />
            ...
            <p className="text-gray-400 text-base sm:text-lg leading-relaxed mb-6 sm:mb-10 max-w-xl">
              Built for Indian businesses with field teams — whether you manage 10 agents or 500+ employees across multiple locations.
            </p>
            <div className="flex flex-col sm:flex-row items-center lg:items-start gap-4 pt-6 border-t border-blue-500/10">
              <div className="flex items-center gap-1.5">
                {[1, 2, 3, 4, 5].map((star) => (
                  <svg key={star} className="w-5 h-5 text-blue-400 drop-shadow-[0_0_8px_rgba(59,130,246,0.4)]" fill="currentColor" viewBox="0 0 20 20">
          ...
          <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="order-2 relative">
            ...
            <div className="space-y-4 sm:space-y-6">
              {benefits.map((benefit, idx) => (
                ...
                >
                  <div className="absolute left-0 top-1/2 -translate-y-1/2 w-14 h-14 rounded-full bg-[#000A1A] border border-blue-500/20 text-blue-400 hidden sm:flex items-center justify-center group-hover:bg-blue-500 group-hover:text-white transition-all duration-500 group-hover:scale-110 z-10">
                    ...
                    <motion.span animate={{ color: ["#93c5fd", "#ffffff", "#93c5fd"] }} transition={{ duration: 4, repeat: Infinity, ease: "easeInOut", delay: idx * 0.4 }} className="relative z-10">
                      {benefit.icon}
                    </motion.span>
                  </div>
                  ...
                  <div className="flex items-center p-5 sm:p-6 rounded-4xl border border-blue-500/10 bg-white/2 hover:bg-white/5 backdrop-blur-xl group-hover:border-blue-500/30 transition-all duration-500 relative overflow-hidden">
                    ...
                    <div className="flex items-center gap-5 sm:gap-0 relative z-10">
                      <div className="p-3 rounded-2xl bg-blue-500/10 text-blue-400 group-hover:bg-blue-500 group-hover:text-white transition-all duration-500 border border-blue-500/20 sm:hidden flex items-center justify-center">
                        {benefit.icon}
                      </div>
