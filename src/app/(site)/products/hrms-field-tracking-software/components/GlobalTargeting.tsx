...
export function GlobalTargeting() {
  ...
  return (
    <section className="relative w-full bg-[#0A0014] overflow-hidden flex flex-col items-center pt-12 sm:pt-24 pb-0 font-sans border-t border-blue-900/20">
      <div className="relative z-20 flex flex-col items-center text-center px-4 max-w-4xl mx-auto mb-8 sm:mb-16">
        ...
        <motion.h3 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold mb-6 leading-tight text-white tracking-tight">
          Powering Workforces{" "}
          <span className="text-transparent bg-clip-text bg-linear-to-r from-blue-400 via-cyan-400 to-blue-500">Across The Globe</span>
        </motion.h3>
        <motion.p initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }} className="text-gray-400 text-lg max-w-3xl mx-auto leading-relaxed">
          Our HRMS platform serves businesses in India, USA, UK, and Canada — with localized compliance, multi-currency payroll, and regional reporting built in.
        </motion.p>
      </div>
      ...
      <div className="relative w-full max-w-[1000px] h-[280px] sm:h-[420px] lg:h-[480px] mx-auto opacity-90 border-t border-white/2">
        ...
        <svg viewBox="0 0 1000 500" className="absolute inset-0 w-full h-full pointer-events-none drop-shadow-[0_0_8px_rgba(59,130,246,0.5)]" preserveAspectRatio="xMidYMid meet">
          {mounted && TARGET_REGIONS.filter((r) => r.id !== "ind").map((region, i) => {
            ...
            return (
              <g key={`connection-${region.id}`}>
                ...
                <motion.path d={d} fill="none" stroke="url(#gradient-hrms)" strokeWidth="2.5" initial={{ pathLength: 0, opacity: 0 }} whileInView={{ pathLength: 1, opacity: 1 }} viewport={{ once: false, amount: 0.1 }} transition={{ duration: 2, delay: i * 0.4, ease: "easeInOut", repeat: Infinity, repeatDelay: 1 }} />
              </g>
            );
          })}
          <defs>
            <linearGradient id="gradient-hrms" x1="0%" y1="0%" x2="100%" y2="0%">
          ...
          <circle cx={HUB.x} cy={HUB.y} r="5" fill="#3b82f6" filter="url(#glow-hrms)" />
          ...
          {TARGET_REGIONS.filter((r) => r.id !== "ind").map((region) => (
            <g key={`node-${region.id}`}>
              <circle cx={region.x} cy={region.y} r="4" fill="#3b82f6" filter="url(#glow-hrms)" />
              <motion.circle cx={region.x} cy={region.y} r="4" fill="none" stroke="#3b82f6" strokeWidth="1.5" animate={{ r: [4, 15], opacity: [1, 0] }} transition={{ duration: 2, repeat: Infinity, ease: "easeOut", delay: 1 }} />
            </g>
          ))}
        </svg>
        ...
        {TARGET_REGIONS.map((region) => {
          ...
          return (
            <div key={`label-${region.id}`} className="absolute transform -translate-x-1/2 -translate-y-full pb-4 pointer-events-none" style={{ left: leftPercent, top: topPercent }}>
              <div className="px-2.5 py-1 bg-[#000A1A]/80 backdrop-blur-md rounded border border-white/10 shadow-[0_4px_10px_rgba(0,0,0,0.5)]">
