...
const WhatsAppButton = () => {
  ...
  useEffect(() => {
    // Only show notification on Home page
    if (pathname === "/") {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-4">
      ...
      <AnimatePresence>
        {(isOpen || showNotification) && (
          <motion.div
            ...
            className="w-72 sm:w-80 overflow-hidden rounded-3xl border border-white/10 bg-[#0B0011]/90 backdrop-blur-2xl shadow-2xl"
          >
            {/* Header */}
      ...
      <motion.button
        ...
        className="relative h-14 w-14 bg-[#25D366] hover:bg-[#128C7E] text-white rounded-full flex items-center justify-center shadow-2xl group transition-all duration-300"
      >
      ...
      <motion.button
        ...
        className="h-14 w-14 bg-[#007AFF] hover:bg-[#0056B3] text-white rounded-full flex items-center justify-center shadow-2xl group transition-all duration-300"
        aria-label="Call us"
      >
        <Phone className="h-7 w-7 transition-transform group-hover:rotate-12" />
        <span className="absolute right-full mr-4 top-1/2 -translate-y-1/2 bg-gray-900/90 backdrop-blur-md text-white text-xs font-bold px-4 py-2 rounded-xl whitespace-nowrap opacity-0 group-hover:opacity-100 transition-all duration-300 translate-x-[10px] group-hover:translate-x-0 pointer-events-none border border-white/10">
